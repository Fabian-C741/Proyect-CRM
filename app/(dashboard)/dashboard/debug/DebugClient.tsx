...
import type { DiagnosticResult } from '@/lib/debug/runner'
...
export default function DebugClient({ timestamp, results }: { timestamp: string; results: DiagnosticResult[] }) {
  const [horaLocal, setHoraLocal] = useState('')

  useEffect(() => {
    setHoraLocal(new Date().toLocaleString('es-AR'))
  }, [])
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <div className="text-xs text-slate-500 font-mono">{horaLocal || 'cargando...'}</div>
          <div className="text-[10px] text-slate-600 font-mono">server: {new Date(timestamp).toLocaleString('es-AR')}</div>
        </div>
      </div>
