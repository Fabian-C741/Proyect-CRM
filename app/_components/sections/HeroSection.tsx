...
export default function HeroSection({ heroTitle, heroSubtitle, heroCtaText, whatsappNumber }: Props) {
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto 5rem' }}>
      <motion.div {...fadeUp(0)} style={{ display: 'inline-flex', alignItems: 'center', padding: '0.375rem 1rem', background: 'rgba(236, 72, 153, 0.1)', border: '1px solid rgba(236, 72, 153, 0.2)', borderRadius: 9999, color: '#f472b6', fontSize: '0.875rem', fontWeight: 600, marginBottom: '1.5rem', letterSpacing: '0.05em' }}>
        ✨ ESTUDIO DE MAQUILLAJE
      </motion.div>
      <motion.h1 {...fadeUp(0.15)} style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 800, lineHeight: 1.1, marginBottom: '1.5rem', letterSpacing: '-0.02em' }}>
        {heroTitle}
      </motion.h1>
      <motion.p {...fadeUp(0.3)} style={{ fontSize: '1.125rem', color: 'var(--text-secondary)', marginBottom: '2.5rem', maxWidth: 600, margin: '0 auto 2.5rem', lineHeight: 1.6 }}>
        {heroSubtitle}
      </motion.p>
